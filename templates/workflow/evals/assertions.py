...
@dataclass
class Assertion:
    """A single check to run against workflow output."""
...
def check_assertion(
    ...
    """Evaluate one assertion against the workflow's output state.

    *result* is expected to be a Pydantic model (WorkflowState subclass)
    or ``None`` if the workflow failed.
    """
    ...
    if atype == "field_equals":
        fld = params["field"]
        expected = params["value"]
        actual = getattr(result, fld, None)
        ok = str(actual) == str(expected)
        return AssertionResult(
            assertion=assertion,
            passed=ok,
            detail=f"{fld}='{actual}' {'==' if ok else '!='} '{expected}'",
        )
