...
Add a new tool to the workflow with the `@tool` decorator. This command walks through designing the tool, generating the code, and verifying it integrates with the tool registry.
...
2. **Who calls it?** This determines visibility:
   - `agent_only` — called by agent Python code in `process()`. The LLM never sees it. Good for: validation, formatting, post-processing, internal state management.
...
- **No side effects on agent state**: Tools should not modify `self.messages` or other agent state. They receive input, do work, and return output. The node's `process()` method decides what to do with the result.
...
If the tool is `agent_only` or `both`, a node's `process()` method may need to call it. Review `src/agent.py` and determine which AgentNode should use the tool. Agent-code tool calls use:

```python
async def process(self, state: MyState) -> MyState:
    result = await self.use_tool("tool_name", param1="value")
    if result.is_error:
        self.logger.warning("Tool failed: %s", result.error)
    else:
        return state.model_copy(update={"output": result.result})
    return state
```

Tools are available to all AgentNode instances in the workflow. Any AgentNode can call any tool — decide which node is the right place based on the workflow's data flow.
...
- Whether `src/agent.py` was updated to call it (and which node)
