...
If the skill should be mentioned in the system prompt so the LLM knows it is available, update `prompts/system.md`. In the workflow template, `build_system_prompt()` is called by AgentNodes that need it — not all nodes use a system prompt. Update only the prompts relevant to nodes that should be aware of the new skill.
