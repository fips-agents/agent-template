...
- **Prompt assembly** is opt-in layered mode via `prompt_assembly:` config block. Four layers with explicit precedence (identity=0, personality=1, governance=2, capabilities=3) assembled into the system message. When config is absent, legacy flat concatenation runs unchanged. `PromptAssembler` in `prompt_assembly.py` handles resolution (identity: inline > file > system prompt fallback; personality: file only, off by default). Structured audit log at `fipsagents.baseagent.prompt_assembly`. Layers 4-6 (knowledge, operational context, ephemeral) are handled by existing mechanisms (memory prefix, AgentState, per-turn injection).
...
- **Session fork and revert** (#168) — `SessionStore.fork(session_id, from_message_index)` branches a session by copying `messages[0:index]` into a new session with `parent_session_id` lineage. `SessionStore.revert(session_id, to_message_index)` truncates in place. REST endpoints: `POST /v1/sessions/{id}/fork` -> `ForkSessionResponse`, `POST /v1/sessions/{id}/revert` -> 204. Cost data is NOT inherited on fork (fresh session). NullSessionStore raises `NotImplementedError`. SQLite truncates for simplicity; Postgres audit-trail revert markers are a future enhancement.
...
- **Event-triggered mode** (#188) — `EventSource` and `EventSink` ABCs in `fipsagents.server.events` following the server-layer Null-implementation + factory pattern. Phase 1a sources: `HttpWebhookSource` (HMAC-SHA256 verification, `asyncio.Queue`, 202 immediate response), `CronSource` (5-field POSIX cron parser, stdlib only). Phase 1a sinks: `NullSink`, `LogSink` (structured JSON), `HttpCallbackSink` (httpx POST, errors swallowed). Phase 1b sources: `KafkaSource` (`fipsagents[kafka]`, aiokafka, consumer group + manual offset commits), `RedisStreamSource` (`fipsagents[redis]`, redis.asyncio, XREADGROUP + XACK). Phase 1b sinks: `KafkaSink` (lazy-init producer), `RedisStreamSink` (XADD with optional trimming). `default_translate_event()` is a standalone server-layer function (NOT on BaseAgent — dependency flows server→baseagent, never reverse). Server starts one `asyncio.Task` per source in `_lifespan()`, processes events through `_process_event()` which reuses `_collect_sync()` (inheriting `_agent_lock`, compaction, permissions, observer chain). Retry with configurable `RetryConfig` (backoff, retriable errors by class name). Session keys use `event:` prefix convention (`event:{path}`, `event:cron:{type}`, `event:kafka:{topic}:{group}`, `event:redis:{stream}:{group}`). `TokenBucketRateLimiter` for per-source rate limiting (defaults: 10/s webhook, 1/s cron). Three new `StreamEvent` variants: `EventReceived`, `EventProcessed`, `EventFailed`. Three new Prometheus metrics: `agent_events_received_total`, `agent_events_processed_total`, `agent_event_processing_duration_seconds`. Config: `server.event_sources` (list, discriminated union on `type`) and `server.event_sink`. Backward compatible: defaults to empty/None. Design doc: `planning/event-triggered-design.md`.
...
- **Trace fidelity levels** — `server.traces.fidelity` controls detail recorded as OTEL span events: `minimal` (default, span attributes only), `standard` (+ message snapshots, tool results), `full` (+ every streaming delta). `OTELTraceStore` exports span events. Configurable via `agent.yaml`.
- **Reducer-based state recovery** (#190) — `AgentState` (Pydantic, `extra="forbid"`) is the typed base for agent-defined state. Subclasses declare `state_type` on BaseAgent and override `reduce(state, event) -> state` (pure sync) and optionally `after_event(state, event)` (async side effects). `StateReducerObserver` wraps the event stream in the observer chain (between metrics and trace collectors). State is per-session: loaded from checkpoint at request start, saved at request end via `checkpoint_state` column on `SessionStore`. Recovery replays events from `TraceStore.list_traces_for_session()` through the reducer — `after_event()` is skipped during replay. Schema mismatch (detected via `state_schema_key()` hash) discards the checkpoint and starts fresh. Config: `server.state_recovery.enabled` (default false). Requires `standard`+ trace fidelity for tool result replay; `full` fidelity for content delta replay.
...
- **Apache AGE graph store** — `GraphStore` ABC in `fipsagents.server.graph_store` with `NullGraphStore` (default no-op) and `AgeGraphStore` (Apache AGE via asyncpg, requires `[graph]` extra). Provides property-graph CRUD: `add_node`, `add_edge`, `get_node`, `get_neighbors`, `query_cypher`, `search_nodes`, `delete_node`, `delete_edge`. AGE requires `LOAD 'age'` + `SET search_path` on each connection (handled via pool `init` callback). `agtype` results are cast to `::text` and JSON-parsed. Cypher values are sanitised through `_escape_cypher_value()` (primitives only). Config: `server.graph.{enabled, backend, database_url, graph_name}` with `database_url` falling back to `storage.database_url`. No stock LLM tool -- agent developers build tools on top. Own asyncpg pool (not shared with PgvectorChunkStore).
- **`GraphConfig`** in `ServerConfig` — `enabled: false`, `backend: "null" | "age"`, `database_url: ""`, `graph_name: "agent_knowledge"`. Factory `create_age_graph_store()` falls back to `NullGraphStore` on any init error (missing asyncpg, bad URL, schema failure). Graph name validated against `^[A-Za-z_][A-Za-z0-9_]*$`.
- **Agent Identity (Kagenti)** — fipsagents delegates agent identity to [kagenti](https://github.com/kagenti/kagenti) rather than implementing its own auth layer. Agents labeled `kagenti.io/type: agent` get auto-registered with Keycloak by the kagenti-operator, which deposits client credentials as a K8s Secret. Token acquisition via OAuth2 Client Credentials Grant; agent→tool delegation via RFC 8693 token exchange with SPIFFE JWT SVIDs. Documented in `docs/architecture.md`. No code implementation -- identity is infrastructure-level (labels + secrets + operator).
- **WorkItemStore** (#215 Phase 1) — `WorkItemStore` ABC in `fipsagents.server.work_items` with `NullWorkItemStore` (default no-op) and `SqliteWorkItemStore` (aiosqlite, `BEGIN IMMEDIATE` for atomic checkout). Lease-based concurrent checkout with periodic expiry sweep. Capability matching (conjunction: agent must satisfy ALL required capabilities with value >= threshold). Data models: `WorkItem`, `WorkItemStatus` (available/checked_out/completed/failed/review_pending/blocked), `Capability`, `HandoffNote` (accomplished/attempted/remaining/blockers/artifacts/context), `Attempt`. Config: `server.work_items.{enabled, backend, lease_duration_seconds, budget_headroom_pct, expire_check_interval_seconds, capabilities}`. Five stock LLM tools auto-registered via `StockToolSpec` when enabled: `check_available_work`, `checkout_work_item`, `complete_work_item`, `release_work_item`, `update_work_progress`. Tools close over `agent._work_item_store` (injected by server before `astep_stream()`). `discover_stock()` extended to handle factory functions returning a list of tools. Six stream events: `WorkItemCheckedOut`, `WorkItemCompleted`, `WorkItemReleased`, `WorkItemFailed`, `BudgetHeadroomWarning`, `HandoffRequired`. Four Prometheus metrics. Session continuity rule + skill templates in agent-loop template. PostgresWorkItemStore and REST endpoints deferred to Phase 2.
- **Server module structure** — `fipsagents.server` is a proper package: `app.py` (OpenAIChatServer), `models.py` (request/response schemas), `sessions.py` (session stores), `tracing.py` (trace model + stores), `collector.py` (TraceCollector), `metrics.py` (Prometheus metrics), `otel.py` (OTEL export), `propagation.py` (W3C Trace Context), `compactor.py` (Compactor ABC), `permissions.py` (PermissionSource ABC), `recovery.py` (state recovery orchestrator), `events.py` (EventSource/EventSink ABCs, models, factories), `sources/` (webhook, cron, null), `sinks/` (null, log, http_callback), `graph_store.py` (GraphStore ABC + AGE backend), `work_items.py` (WorkItemStore ABC + NullWorkItemStore + factory), `work_item_stores/` (sqlite), `sqlite.py` (shared connection manager). `__init__.py` re-exports `OpenAIChatServer`, `ChatCompletionRequest`, `ChatMessage`.
