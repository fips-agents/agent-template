...
- **Compactor ABC** in `fipsagents.server.compactor` — `should_compact()` / `compact()` methods. `NullCompactor` (default no-op). Server skips compaction when pending state exists. `CompactionStarted`/`CompactionCompleted`/`CompactionSkipped` stream events. LLM-driven implementation deferred to #166.
