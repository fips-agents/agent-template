...
Platform mode: opt-in delegation of LLM orchestration to OGX (LlamaStack rebrand) via `client.responses.create()`. Closes [#154](https://github.com/fips-agents/agent-template/issues/154).

### Added

- **`platform:` config block on `AgentConfig`** ([#154](https://github.com/fips-agents/agent-template/issues/154)). New `PlatformConfig` (with nested `PlatformMcpServer` and `ModerationConfig`) opts into OGX's `/v1/responses` endpoint instead of `/v1/chat/completions`. When `platform.enabled: true`, OGX orchestrates MCP tool calls, shield enforcement, and the inference loop server-side; the framework skips its own `connect_mcp()` startup loop. Fully opt-in — `enabled` defaults to `false` and existing chat-completions agents are unchanged.
- **`PlatformMcpServer` accepts both reference modes**. Each MCP entry needs `name` (becomes `server_label` on the wire) plus exactly one of `connector_id` (references a connector pre-registered in OGX's stack YAML) or `url` (inline `server_url` passed on every Responses request). Optional `authorization` field forwards a bearer token for OAuth-protected MCP servers. Exactly-one-of validator catches misconfigs at config-load time, not on first call.
- **`LLMClient.call_model_responses()`** ([#154](https://github.com/fips-agents/agent-template/issues/154)). Non-streaming `/v1/responses` call returning a `PlatformResponse` (`content`, `refusal`, `usage`, `response_id`, `raw`). Defaults `tools` to `_mcp_servers_to_tools(platform.mcp)` and `guardrails` to `platform.guardrails`; both can be overridden per-call. `guardrails` travels via `extra_body` because the OpenAI SDK rejects unknown top-level kwargs.
- **`LLMClient.call_model_responses_stream()`** ([#154](https://github.com/fips-agents/agent-template/issues/154)). Maps OGX's Responses SSE event protocol onto the existing `StreamEvent` taxonomy: `response.output_text.delta` → `ContentDelta`; a `refusal` in the terminal `response.completed` payload → the new `GuardrailFiredEvent` (with `shield_id` parsed from "(flagged for: ...)" when present, else comma-joined configured shields) followed by `StreamComplete(finish_reason="guardrail")`.
- **`LLMClient.moderate()`** ([#154](https://github.com/fips-agents/agent-template/issues/154)). Wraps `/v1/moderations` and returns a `ModerationResult` aggregating `flagged` / `categories` / `category_scores` across results. Observability-only — never blocks. `BaseAgent.moderate(content, *, model=None)` exposes the same call and emits a structured log line for the audit trail; defaults `model` to the first entry in `platform.guardrails`.
- **`GuardrailFiredEvent` `StreamEvent` variant** ([#154](https://github.com/fips-agents/agent-template/issues/154)). Carries `shield_id`, `action` (`"blocked"` / `"warned"`), optional `category`, and optional `message`. Additive to the union — existing serializers and the `TraceCollector` are unchanged.
- **`platform_live` pytest mark** ([#154](https://github.com/fips-agents/agent-template/issues/154)). New live integration test at `tests/integration/test_platform_live.py` round-trips streaming, non-streaming, guardrail, and moderation paths against a real OGX endpoint. Skipped automatically when `OGX_ENDPOINT` is unset; configurable via `OGX_MODEL` and `OGX_SHIELD`. Catches SDK kwarg-validation issues that mocks miss (eg the `extra_body` requirement for `guardrails`).
- **`platform:` stanza in the agent-loop template `agent.yaml`** ([#154](https://github.com/fips-agents/agent-template/issues/154)). Production env vars: `PLATFORM_MODE` (default `false`), `OGX_ENDPOINT` (required when enabled). Misconfig of both `platform.mcp` and the legacy `mcp_servers:` block surfaces a structured log line at setup, not silent precedence.

### Notes

- **Streaming + late-firing output shields can leak pre-shield content.** When OGX's post-generation shield fires, the unsafe `output_text.delta` events have already been streamed; the framework passes them through to consumers. The terminal `response.completed` payload then replaces the streamed text with the refusal and we emit `GuardrailFiredEvent` + `finish_reason="guardrail"`. This matches how hosted providers like Anthropic and OpenAI handle late-firing safety filters. Consumers that need post-shield content only should buffer until `StreamComplete`.
- **OGX wire-format quirks captured during implementation.** No dedicated guardrail event type — refusals are signaled structurally via `output[*].content[*].type == "refusal"` on the terminal payload. No machine-readable shield ID in the response — the violation type is embedded in the refusal string (eg `"(flagged for: insecure-eval-use)"`); the framework parses it heuristically with a fallback to the configured shield list.
- **Decoupled from #81 (observability).** The `platform:` block does not assume an `observability:` block exists. When #81 lands, OTLP wiring will apply uniformly to both chat-completions and Responses-based agents.
- **Cluster-smoked end-to-end.** Verified against a live OGX endpoint with the `code-scanner` shield: streaming + input-shield trigger emits zero content deltas before the guardrail event; non-streaming + output-shield trigger replaces `content` with `refusal`; moderations return `flagged=False` for benign input.
